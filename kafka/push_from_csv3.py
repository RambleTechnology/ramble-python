...
def read_csv(file_path):
    ...
    try:
        ...
        if not reader.fieldnames or "sn" not in reader.fieldnames:
            raise ValueError("CSV 缺少必需列: sn")
        for index, row in enumerate(reader, start=1):
            try:
                ...
                if not sn:
                    raise ValueError(f"第{index}行 sn 为空")
                if sn in seen_sn:
                    raise ValueError(f"第{index}行 sn 重复: {sn}")
                seen_sn.add(sn)
