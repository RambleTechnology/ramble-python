...
class App:
    def __init__(self, root):
        ...
        self.current_config = {
            ...
            "retries": 0,  # 增加重试次数
            "socket.timeout.ms": 5000,  # 增加超时时间
            "request.timeout.ms": 5000,  # 增加超时时间
            "reconnect.backoff.ms": 1000,  # 设置合理的重连间隔
    ...
    def connect_kafka(self):
        ...
        if self.is_connected:
            ...
            self.connect_button.config(text="连接Kafka")
            self.send_button.config(state=tk.DISABLED)
            self.log("已断开Kafka连接")
            return

        # 尝试连接
        self.log(f"正在连接到Kafka服务器: {self.current_config['bootstrap.servers']}")
        ...
        try:
            ...
            # 尝试获取元数据来验证连接
            self.log(f"尝试获取元数据来验证连接")
            metadata = self.producer.list_topics(timeout=1.0)
            if metadata:
                ...
                self.log(f"元数据获取成功。orig_broker_name={metadata.orig_broker_name}；brokers={metadata.brokers};主题={metadata.topics}")                
                self.log("连接成功！！！")
            else:
                self.log("无法获取Kafka元数据")
                self.connection_status_var.set("连接失败")
                self.connection_status_label.configure(foreground="red")
                self.producer = None
        except KafkaError as e:
            self.log(f"Kafka连接错误: {e}")
            self.connection_status_var.set("连接失败")
            self.connection_status_label.configure(foreground="red")
            self.producer = None
        except Exception as e:
            self.log(f"连接过程中发生错误: {e}")
            self.connection_status_var.set("连接失败")
            self.connection_status_label.configure(foreground="red")
            self.producer = None

    # 这个 回调是全局的，连接kafka，发送消息如果发生错误，都会回调到这里。
    def error_callback(self, err):
        if err.code() == KafkaError._ALL_BROKERS_DOWN:
            self.log(f"连接错误: 所有代理都不可用 - {err}")
            if self.is_connected:
                self.is_connected = False
                self.connection_status_var.set("连接已断开")
                self.connection_status_label.configure(foreground="red")
                self.connect_button.config(text="连接Kafka")
                self.send_button.config(state=tk.DISABLED)
        else:
    ...
    def _delivery_report(self, err, msg):
        if err:
            self.log(f"[消息发送回调]消息发送失败: {err}, 主题: {msg.topic()}")
        else:
            self.log(
                f"[消息发送回调]消息成功发送到 {msg.topic()}[{msg.partition()}],"
    ...
    def create_widgets(self):
        ...
        self.connection_status_label.grid(row=0, column=3, sticky=tk.W, pady=5, padx=10)
        ...
        self.connect_button.grid(row=0, column=2, rowspan=2, padx=10, pady=5)
